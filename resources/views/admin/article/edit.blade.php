...
@section('content')
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Update your article information</p>
                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                            ...
                            <div id="breakingTitleField" class="md:col-span-2 lg:col-span-4 {{ $article->is_breaking ? '' : 'hidden' }}">
                                <label for="breaking_title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Breaking News Title</label>
                                <input
                                    type="text"
                                    id="breaking_title"
                                    name="breaking_title"
                                    value="{{ old('breaking_title', $article->breaking_title) }}"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="w-full h-48 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center mb-4 overflow-hidden" id="mainImagePreview">
                                    @if($article->image)
                                        <img src="{{ asset('storage/' . $article->image) }}" id="mainPreviewImg" class="w-full h-full object-cover" alt="Main Image">
                                    @else
                                ...
                                <div class="drag-area w-full p-4 border-2 border-dashed @error('image') border-red-500 @else border-gray-300 dark:border-gray-600 @enderror rounded-lg text-center hover:border-indigo-400 cursor-pointer" onclick="document.getElementById('mainImage').click()">
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400">Click to upload new image</p>
                                    <p class="text-xs text-gray-500 mt-1">PNG, JPG up to 2MB</p>
                                </div>
                                @if($article->image)
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">Current: {{ basename($article->image) }}</p>
                                @endif
                                @error('image')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="md:col-span-2">
                                ...
                                <div class="w-full h-48 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center mb-4 overflow-hidden" id="featuredImagePreview">
                                    @if($article->featured_image)
                                        <img src="{{ asset('storage/' . $article->featured_image) }}" id="featuredPreviewImg" class="w-full h-full object-cover" alt="Featured Image">
                                    @else
                                ...
                                @if($article->featured_image)
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">Current: {{ basename($article->featured_image) }}</p>
                                @endif
                        ...
                        <div class="mb-6 md:col-span-2 lg:col-span-4">
                            ...
                            <input
                                id="tags"
                                name="tags"
                                value="{{ old('tags', $article->tags ? $article->tags->pluck('name')->implode(',') : '') }}"
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm p-6">
                        <div class="flex flex-col sm:flex-row gap-3 justify-between">
                            ...
                            <div class="flex gap-3">
                                <button type="button" onclick="if(confirm('Are you sure you want to delete this article?')) { document.getElementById('delete-form').submit(); }" class="px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 flex items-center justify-center">
        ...
        <!-- Delete Form -->
        <form id="delete-form" action="{{ route('articles.destroy', $article->id) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
        ...
        document.getElementById('mainImage').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    const icon = document.getElementById('mainDefaultIcon');
        ...
        document.getElementById('featuredImage').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    const icon = document.getElementById('featuredDefaultIcon');
                    img.src = e.target.result;
                    img.classList.remove('hidden');
                    if(icon) icon.classList.add('hidden');
                }
